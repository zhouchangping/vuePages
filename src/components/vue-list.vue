...
export default {
  ...
  mounted () {
    this.counts = this.list.length - 1;
    this.initData();
    this.handleScroll();
  },
  methods: {
    ...
    handleScroll () {
      ...
      if (this.lastScrollTop === null || Math.abs(_scrollTop - this.lastScrollTop) > this._max) { // 头部区域超过11*44
        this.lastScrollTop = _scrollTop; // 开始为0
      } else {
