...
allEntryInputs = args["_"].slice(2);


if (isDev) {
  allEntryInputs = args["_"].slice(3); // 新增分模块打包--参数获取
}
console.log("----------------------------------")
console.log(allEntryInputs)

let newArr = allEntryInputs.map(function (item) {
  return srcDir + "/" + "pages" + "/" + item + "/" + item + ".html";
});
if (!newArr.length) {
  newArr = glob.sync(srcDir + "/pages/**?/*.html");
}
console.log(newArr)

let HtmlWebpackPlugins = []; // webpack插件
//配置pages多页面获取当前文件夹下的html和js
function getEntry (allEntrys) {
  let entries = {};
  allEntrys.forEach(function (entry) {
    ...
    HtmlWebpackPlugins.push(
      new HtmlWebpackPlugin({
        chunks: [entry], // 引入自己的,不然每个页面都是所有的js和css
    ...
    entries[entry] = "@/" + "pages" + "/" + entry + "/" + entry + ".js"
  });
  return entries;
}
