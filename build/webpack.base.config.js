...
if (argsEnd.length === 0) {
  ...
  argsEnd = glob.sync(srcDir + "/pages/**?/*.html").map((entry) => {
    return entry.split("/").splice(-3)[1];
  });
}
argsEnd.forEach(moduleName => {
  console.log(moduleName)
  const moduleDetails = utils.getMultiEntry(`./src/pages/**?/${moduleName}.js`);
  if (Object.keys(moduleDetails).length == 0) {
    console.log(chalk.red(`Error: this module(${chalk.yellow(moduleName)}) could not be found`))
  }
  entries[moduleName] = moduleDetails[moduleName];

  HtmlWebpackPlugins.push(
    new HtmlWebpackPlugin({
      chunks: [moduleName], // 引入自己的,不然每个页面都是所有的js和css
      title: '从零配置webpack4+vue脚手架',
      filename: moduleName+ '.html', // 在dist下生成index.html
      template: path.join(__dirname, "../src/" + "pages" + "/" + moduleName + "/" + moduleName + ".html") // 插件将会自动向这个模版文件中注入打包后的 js 'css' 文件资源
    })
  )
})
...
const webpacks = {
  ...
  output: {
    ...
    // 此选项决定了每个输出 bundle 的名称。这些 bundle 将写入到 output.path 选项指定的目录下   publicPath + /js/name.///    https://juejin.cn/post/6844903542893854734
    // filename: 'js/[name].[hash].js', // hash是跟整个项目的构建相关，只要项目里有文件更改，整个项目构建的hash值都会更改，并且全部文件都共用相同的hash值
    // filename: 'js/[name].[chunkhash].js',
    filename: 'js/[name].[md5:contenthash:hex:8].js',
    // http://react-china.org/t/webpack-output-filename-output-chunkfilename/2256/2
    // chunkFilename: 'js/[name].js' // 此选项决定了非初始（non-initial）chunk 文件的名称,这些文件名需要在运行时根据 chunk 发送的请求去生成。因此，需要在 webpack runtime 输出 bundle 值时，将 chunk id 的值对应映射到占位符(如 [name] 和 [chunkhash])。这会增加文件大小，并且在任何 chunk 的占位符值修改后，都会使 bundle 失效。各页面的chunkjs
  },
  ...
  resolve: {
    ...
    alias: {
      ...
      // 'vue$': 'vue/dist/vue.esm.js' // 完整版本。高版本的vue默认是runtime版本，是不太编译器的，无法识别vue语法，需要在webpack配置带编译器的完整版本
    }
  },
  // target: "web",
  module: {
    rules: [
      ...
      // markdown 二
      {
        ...
        use: [
          ...
          {
            loader: require.resolve('../loader/markdown-loader.js'),
          },
        ],
      },
      ...
      {
        test: /\.css|scss$/, // 先补全浏览器前缀，再解析sass、less，再使用css加载器处理@import,最后使用style加载器将样式文件插入head标签
        use: [
          // 它会应用到普通的 `.css` 文件
          // 以及 `.vue` 文件中的 `<style>` 块
          ...
          // "style-loader", // 和min冲突   // 将 JS 字符串生成为 style 节点
  ...
  optimization: {
    ...
    splitChunks: {
      ...
      cacheGroups: { // 缓存组可以继承和/或覆盖splitChunks.*;中的任何选项。但是test，priority并且reuseExistingChunk只能在高速缓存组级别配置。要禁用任何默认缓存组，请将它们设置为false
        ...
        // },
        styles: { // 加上避免Cannot read property 'pop' of undefined
          name: 'styles',
          test: /\.(sa|sc|c)ss$/,
          chunks: 'all',
          enforce: true,
        ...
        //   priority: 20
        // },
        commons: { // 分离公共文件（CSS和JS）
          // test: /[\\/]src[\\/]common[\\/]/, // 内包含vue代码，所以不能test，src
          name: 'commons', // 如公用的header组件
